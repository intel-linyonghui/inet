...
of \textit{Unit Disk Graph}, a model widely used the study of wireless ad-hoc networks.
It is applicable if network nodes need to have a finite communication range, 
but physical effects of signal propagation are to be ignored.
...
APSK radio models provides a hypothetical radio that simply uses 
one of the well-known modulations without utilizing other techiques 
such as forward error correction, interleaving, spreading, etc. 
(APSK stands for Amplitude and Phase-Shift Keying.)

variants of ApskRadio (Scalar, Dimensional, Layered):
\nedtype{ApskScalarRadio}, \nedtype{ApskDimensionalRadio},
\nedtype{ApskLayeredRadio}

It must be used in conjunction with the ~ApskScalarRadioMedium model.
 
\nedtype{ApskScalarRadio} contains \nedtype{IsotropicAntenna},
\nedtype{ApskScalarTransmitter}, and \nedtype{ApskScalarTransmitter}.
